...
namespace CSharpUtilsNETStandard.Utils.Extensions.Collections
{
    ...
    public static class CollectionUtilExtensions
    {
        ...
        [NotNull]
        public static IEnumerable<TResult> SelectWhereNotNull<TSource, TResult>([NotNull] this IEnumerable<TSource> source, [NotNull] Func<TSource, TResult> selector)
            where TResult : class
        {
        ...
        public static bool ContainsBy<T, TComparedValue>([NotNull] this IEnumerable<T> source, [NotNull] T value, [NotNull] Func<T, TComparedValue> selector)
            where TComparedValue : IEquatable<TComparedValue>
        {
            return source.Select(selector).Contains(selector(value));
        }

        public static void RemoveNullValues<TKey, TValue>([NotNull] this IDictionary<TKey, TValue> dictionary)
            where TValue : class
        {
        ...
        [NotNull, ItemNotNull]
        public static IEnumerable<T> PrintWarningPredicate<T>([NotNull, ItemNotNull] this IEnumerable<T> enumerable, [NotNull] Predicate<T> predicate, [NotNull] Func<T, string> warningMessage)
        {
        ...
        [NotNull]
        public static Dictionary<TValue, List<TKey>> ToValueKeyListDictionary<TKey, TValue>([NotNull] this IDictionary<TKey, TValue> dictionary)
        ...
        [NotNull, ItemNotNull]
        public static List<List<T>> GroupBySimple<T>([NotNull, ItemNotNull] this IEnumerable<T> source, [NotNull] Func<T, T, bool> belongsToSameGroupPredicate)
        {
        ...
        {
            return entry.Key is string && string.IsNullOrWhiteSpace(entry.Key.ToString())
                || entry.Value == null
                || entry.Value is string && string.IsNullOrWhiteSpace(entry.Value.ToString());
        }
        ...
        private static bool RemoveNullOrWhiteSpaceKeysAndValues([NotNull] this ICollection collection)
        {
            ...
            if (!(collection is IList list)) return false;
            foreach (object o in list)
            {
                switch (o)
                {
                    case null:
                    case string _ when string.IsNullOrWhiteSpace(o.ToString()):
                        numberOfNullEntries++;
                        break;
                    case ICollection castCollection:
                        anyNullValues |= castCollection.RemoveNullOrWhiteSpaceKeysAndValues();
                        break;
                }
            }
        ...
        /// <param name="collection"></param>
        /// <returns></returns>
        public static bool RemoveNullOrWhiteSpaceKeysAndValues<T>([NotNull, ItemCanBeNull] this ICollection<T> collection)
            where T : class
        {
            if (collection.Count == 0) return false;
            bool anyNullValues = false;
        ...
        /// <returns></returns>
        public static bool RemoveNullOrWhiteSpaceKeysAndValues<TKey, TValue>([NotNull] this IDictionary<TKey, TValue> dictionary)
            where TValue : class
        {
        ...
        public static void AddIfNotContains<T>([NotNull] this IList<T> list, [NotNull, ItemNotNull] IEnumerable<T> values)
        {
        ...
        [CanBeNull]
        public static TValue GetValueOrNull<TKey, TValue>([NotNull] this IReadOnlyDictionary<TKey, TValue> dictionary, [NotNull] TKey key, [CanBeNull] LogLevel? failureLogLevel = null)
            where TValue : class
        {
        ...
        {
            if (failureLogLevel.HasValue) Logger.PrintLogLevel(failureLogLevel.Value, $"The key {key} was not found in a Dictionary of <{typeof(TKey).Name}, {typeof(TValue).Name}>! This warning message is for debugging purposes and could be normal behaviour.", nameof(CollectionUtilExtensions));
        }
